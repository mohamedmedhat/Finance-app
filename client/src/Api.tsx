...
export const searchCompanies = async (query: string) => {
  try {
    const data = await axios.get<searchRespones>(
      `https://financialmodelingprep.com/api/v3/search?query=${query}&limit=10&exchange=NASDAQ&apikey=${process.env.REACT_API_KEY}`
    );
    return data;
  } catch (error) {
    if (axios.isAxiosError(error)) {
      ...
      return `un expected error happend`;
    }
  }
};
